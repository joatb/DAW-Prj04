...
class StarHunter {
  ...
  spawn() {
    ...
    $("#taula-puntuacions")
      .find("tbody")
      .append(`<tr id="puntuacio-${this.id}"><td>${this.id}</td><td>${this.stars.length}</tr>`);
    this.obj = $(`#${this.id}`);
...
function init() {
  $("#engegar").click(startStop);

  $("#aturar").click(startStop);


  ws.onopen = () => {
    var m = { event: "crearAdmin" };
